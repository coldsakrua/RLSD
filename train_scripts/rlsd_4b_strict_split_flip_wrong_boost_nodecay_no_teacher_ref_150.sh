#!/bin/bash
#SBATCH -o logs/rlsd_4b_strict_split_flip_wrong_boost_nodecay_no_teacher_ref_150.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
#SBATCH --exclude=gpua800n15,gpua800n21,gpua800n26
...
OUTPUT_DIR=${OUTPUT_DIR:-${BASE_DIR}/outputs/rlsd_4b_strict_split_flip_wrong_boost_nodecay_no_teacher_ref_150}
RUN_CONFIG=${RUN_CONFIG:-rlsd_4b_strict_split_flip_wrong_boost_nodecay_no_teacher_ref_150}
...
# Steps [0, decay_steps): same as main OPSD (all groups). From decay_steps on: no teacher token-gap; mixed-only GRPO.
TOKEN_GAP_LAMBDA=${TOKEN_GAP_LAMBDA:-1.0}
TOKEN_GAP_DECAY_STEPS=${TOKEN_GAP_DECAY_STEPS:-150}
TOKEN_GAP_DECAY_LINEAR=${TOKEN_GAP_DECAY_LINEAR:-false}
STRICT_SPLIT_GRPO_MIXED_ONLY_AFTER_DECAY=${STRICT_SPLIT_GRPO_MIXED_ONLY_AFTER_DECAY:-true}
...
echo "[schedule] OPSD steps 0..$((TOKEN_GAP_DECAY_STEPS - 1)) = main ref; step>=${TOKEN_GAP_DECAY_STEPS}: mixed-only GRPO (no teacher token-gap)"
echo "[ablation] strict_split_grpo_mixed_only_after_decay=${STRICT_SPLIT_GRPO_MIXED_ONLY_AFTER_DECAY}"
...
CUDA_VISIBLE_DEVICES="${TRAIN_CUDA_VISIBLE_DEVICES}" accelerate launch \
    ...
    --token_gap_decay_linear "${TOKEN_GAP_DECAY_LINEAR}" \
    --strict_split_grpo_mixed_only_after_decay "${STRICT_SPLIT_GRPO_MIXED_ONLY_AFTER_DECAY}" \
    --fixed_teacher false \
