#!/bin/bash
#SBATCH -o logs/rlsd_4b_strict_split_flip_mixed_only_nodecay_no_teacher_ref_50.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
#SBATCH --exclude=gpua800n08,gpua800n09,gpua800n21,gpua800n06
...
OUTPUT_DIR=${OUTPUT_DIR:-${BASE_DIR}/outputs/rlsd_4b_strict_split_flip_mixed_only_nodecay_no_teacher_ref_50}
RUN_CONFIG=${RUN_CONFIG:-rlsd_4b_strict_split_flip_mixed_only_nodecay_no_teacher_ref_50}
...
# Full OPSD (lambda=1) for steps [0, decay_steps); from decay_steps on -> GRPO-only (lambda=0).
# No linear ramp: token_gap_decay_linear=false.
...
echo "[ablation] strict_split_mixed_only=${STRICT_SPLIT_MIXED_ONLY} token_gap_decay_steps=${TOKEN_GAP_DECAY_STEPS}"
echo "[ablation] teacher_include_reference_solution=${TEACHER_INCLUDE_REFERENCE_SOLUTION}"
echo "[schedule] token_gap_lambda=${TOKEN_GAP_LAMBDA} decay_steps=${TOKEN_GAP_DECAY_STEPS} decay_linear=${TOKEN_GAP_DECAY_LINEAR} (linear=false: full OPSD until step ${TOKEN_GAP_DECAY_STEPS}, then off)"
