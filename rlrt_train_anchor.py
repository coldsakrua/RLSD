...
from rlrt_trainer import RLRTTrainer
...
class ScriptArguments:
    ...
    token_gap_lambda: float = 0.5
    token_gap_decay_steps: int = 0
    token_gap_decay_linear: bool = True
    rollout_filter: str = "all"
    fixed_teacher: bool = False
    teacher_update_interval_steps: int = 0
    ...
    teacher_include_reference_solution: bool = False
    # RLRT paper defaults for base models: lambda_init=0.5, epsilon_w=1.0,
    # no lambda decay. The shell script sets token_gap_lambda/decay accordingly.
    rlrt_weight_clip: float = 1.0
    rlrt_delta: Optional[float] = 2.0
    rlrt_teacher_context_mode: str = "successful_rollout"
    rlrt_teacher_prompt_template_with_rollout: str = (
        "{prompt}\n\n[Correct rollout]\n{correct_rollout}\n\n[Student response]\n"
    )
    ...
    # RLRT clips the token weight, not the final advantage.
...
def main():
    ...
    logging_setup = configure_wandb_offline(
        ...
        extra_meta={
            ...
            "rlrt_weight_clip": float(script_args.rlrt_weight_clip),
            "rlrt_delta": None if script_args.rlrt_delta is None else float(script_args.rlrt_delta),
            "rlrt_teacher_context_mode": script_args.rlrt_teacher_context_mode,
        },
    )
    ...
    print(
        f"[rlrt] weight_clip={script_args.rlrt_weight_clip} "
        f"teacher_context_mode={script_args.rlrt_teacher_context_mode}",
        flush=True,
    )

    if script_args.dapo_epsilon_high is not None:
        setattr(training_args, "epsilon_high", float(script_args.dapo_epsilon_high))
    if script_args.rlrt_delta is not None:
        setattr(training_args, "delta", float(script_args.rlrt_delta))
    ...
    trainer = RLRTTrainer(
        ...
        rlrt_weight_clip=script_args.rlrt_weight_clip,
        rlrt_teacher_context_mode=script_args.rlrt_teacher_context_mode,
        rlrt_teacher_prompt_template_with_rollout=script_args.rlrt_teacher_prompt_template_with_rollout,
