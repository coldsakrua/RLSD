#!/bin/bash
#SBATCH -o /gpfs/share/home/2501210611/RLSD/verl_logs/train/cast_teacher50_4b_full_noflip_withgap005.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
#SBATCH --exclude=gpua800n07,gpua800n09,gpua800n10,gpua800n11,gpua800n25

set -eo pipefail

# Same as cast_ema_4b_256_noflip.sh, but teacher RLSD shaping covers the full rollout
# (all MAX_COMPLETION_LENGTH response tokens), not only the first 256.
# Teacher uses the exact same prompt tokens as the student; weights follow EMA of student LoRA.
#
# Note: teacher_shaping_length_cap=0 falls back to max_teacher_prompt_length in code, so we
# explicitly set the cap to MAX_COMPLETION_LENGTH for full-response shaping.
RUN_CONFIG="${RUN_CONFIG:-cast_teacher50_4b_full_noflip_withgap005}"
