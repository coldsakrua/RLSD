#!/bin/bash
#SBATCH -o /gpfs/share/home/2501210611/RLSD/verl_logs/train/cast_ema_4b_full1200_withgap005.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
#SBATCH --exclude=gpua800n07,gpua800n23,gpua800n25,gpua800n10,gpua800n11

set -eo pipefail

# VERL port of rlsd_4b_strict_split_flip_wrong_boost_nodecay_no_teacher_ref:
# strict-split sign-flip + wrong-path positive boost, weight_decay=0, no frozen ref teacher.
# Teacher weights follow EMA of student LoRA (identical_student prompt, internal teacher logprobs).
#
# vs cast_ema_4b_512.sh: teacher RLSD shaping applies to every response token
# (TEACHER_SHAPING_LENGTH_CAP = MAX_COMPLETION_LENGTH), not only the first 512.
#
# Note: teacher_shaping_length_cap=0 falls back to max_teacher_prompt_length in code.
RUN_CONFIG="${RUN_CONFIG:-cast_ema_4b_full1200_withgap005}"
...
TOKEN_GAP_DECAY_STEPS="${TOKEN_GAP_DECAY_STEPS:-1200}"
