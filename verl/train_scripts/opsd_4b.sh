#!/bin/bash
#SBATCH -o /gpfs/share/home/2501210611/RLSD/verl_logs/train/verl_opsd_4b.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
#SBATCH --exclude=gpua800n04,gpua800n11,gpua800n26

set -eo pipefail

# Official OPSD-style run: teacher/reference distillation only, n=1.
RUN_CONFIG="${RUN_CONFIG:-verl_opsd_4b}"
ADV_ESTIMATOR="${ADV_ESTIMATOR:-opd_zero}"
REWARD_FUNCTION_NAME="${REWARD_FUNCTION_NAME:-compute_score_zero}"
TOKEN_GAP_LAMBDA="${TOKEN_GAP_LAMBDA:-0.0}"
...
OFFICIAL_TEACHER_PROMPT="${OFFICIAL_TEACHER_PROMPT:-true}"
DISTILLATION_LOSS_COEF="${DISTILLATION_LOSS_COEF:-1.0}"
DISTILLATION_USE_TASK_REWARDS="${DISTILLATION_USE_TASK_REWARDS:-false}"
...
# n=1 uses much less rollout memory than RLSD/RLRT, so default to 8 prompts.
