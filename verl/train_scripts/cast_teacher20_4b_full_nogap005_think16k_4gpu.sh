#!/bin/bash
#SBATCH -o /gpfs/share/home/2501210611/RLSD/verl_logs/train/cast_teacher20_4b_full_nogap005_think16k_4gpu.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:4
#SBATCH --mem-per-cpu=163840M
...
INTERNAL_TEACHER_BASE_UNTIL_STEP="${INTERNAL_TEACHER_BASE_UNTIL_STEP:-20}"
INTERNAL_TEACHER_SNAPSHOT_INTERVAL="${INTERNAL_TEACHER_SNAPSHOT_INTERVAL:-20}"
...
export RLSD_DISCARD_TRUNCATED_COMPLETIONS="${RLSD_DISCARD_TRUNCATED_COMPLETIONS:-true}"
# Teacher sees the full student rollout (thinking + answer), not just the first full tokens.
...
AGENT_LOOP_WORKERS="${AGENT_LOOP_WORKERS:-12}"
...
export DISABLE_THINKING_IN_CHAT_TEMPLATE="${DISABLE_THINKING_IN_CHAT_TEMPLATE:-false}"
