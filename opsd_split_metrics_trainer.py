...
class OPSDSplitMetricsTrainer(RLSDTrainer):
    ...
    def _generate_and_score_completions(self, inputs):
        ...
        self._log_metric("reward_mean_mixed_correct", reward_mean_mixed_correct)
        self._log_metric("reward_mean_mixed_wrong", reward_mean_mixed_wrong)
        ...
        self._log_metric("completion_correct_frac", float(sample_correct.float().mean().item()))
        self._log_metric("completion_wrong_frac", float(sample_wrong.float().mean().item()))
        self._log_metric("completion_mixed_correct_frac", float(mixed_correct.float().mean().item()))
        self._log_metric("completion_mixed_wrong_frac", float(mixed_wrong.float().mean().item()))

        seq_adv = (token_adv.sum(dim=1) / completion_mask_f.sum(dim=1).clamp(min=1.0)).detach()
        self._log_vector_stats_full("seq_adv", seq_adv)

        token_count = completion_mask_f.sum().clamp(min=1.0)
        adv_abs_mean = float((token_adv.abs().sum() / token_count).item())
        self._log_metric("adv_abs_mean", adv_abs_mean)
        self._log_masked_stats_full("token_adv", token_adv, completion_mask_f)
        ...
        self._log_metric("answer_weight_mean", 1.0)
        self._log_metric("completion_len_mean", float(lens.mean().item()))
        self._log_metric("completion_len_min", float(lens.min().item()))
        self._log_metric("completion_len_max", float(lens.max().item()))
        self._log_metric("terminated_len_mean", float(snap_lens.mean().item()))
        self._log_metric("ended_with_eos_frac", eos_frac)
        self._log_metric("no_eos_frac", 1.0 - eos_frac)
        return batch
