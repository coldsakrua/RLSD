#!/bin/bash
#SBATCH -o logs/rlsd_8b_paper.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
OUTPUT_DIR=${OUTPUT_DIR:-${BASE_DIR}/outputs/rlsd_8b_paper}
RUN_CONFIG=${RUN_CONFIG:-rlsd_8b_paper}
...
if [ "${USE_SIGN_CONSTRAINED_FALLBACK}" = "true" ]; then
    ...
    echo "[warn] For canonical RLSD use: USE_SIGN_CONSTRAINED_FALLBACK=false (default in rlsd_8b_paper.sh)."
fi
...
echo "[rlsd_paper] model=8b trainer=RLSDTrainer fallback=${USE_SIGN_CONSTRAINED_FALLBACK} lmbda=${LMBDA} decay=${LMBDA_DECAY_STEPS} eps_w=${JSD_TOKEN_CLIP}"
