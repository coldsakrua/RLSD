#!/bin/bash
#SBATCH -o logs/analyze_opsd_token_shift_no_privileged.%j.out
#SBATCH -p GPUA800
#SBATCH --nodes=1
...
OUTPUT_JSON=${OUTPUT_JSON:-${OUTPUT_DIR}/opsd_token_shift_no_privileged_${RUN_TAG}.json}
mkdir -p "${OUTPUT_DIR}"
...
# Fully aligned mode: teacher reuses student prompt prefix ids.
TEACHER_USE_STUDENT_PROMPT=${TEACHER_USE_STUDENT_PROMPT:-true}
...
echo "[analyze] teacher_use_student_prompt=${TEACHER_USE_STUDENT_PROMPT}"
...
python analyze_opsd_token_shift.py \
    ...
    --prompt_prefix "${PROMPT_PREFIX}" \
    --prompt_suffix "${PROMPT_SUFFIX}" \
    --teacher_use_student_prompt "${TEACHER_USE_STUDENT_PROMPT}" \
